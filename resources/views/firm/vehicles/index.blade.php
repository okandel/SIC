...
@section('breadcrumbs')
    <div class="row">
        <div class="col-sm-3">
            <h4 class="ml-4 mt-3"><i class="fa fa-car fa-1x"></i> Vehicles</h4>
        </div>
        <div class="col-sm-9">
            <ul class="breadcrumb">
                <li class="breadcrumb-item float-md-right ml-3 mr-4">
                    <p>{{ Breadcrumbs::render('vehicles') }}</p>
                </li>
            </ul>
        </div>
    </div>
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="body row" style="margin-bottom: -15px">
                    <div class="col-sm-9">
                        <form method="get" id="form_vehicles_search">
                            <div class="row clearfix">
                    ...
                    <div class="col-sm-2 m-auto">
                        <a style="float: right; margin-top: 5px; margin-bottom: 20px" class="btn btn-success" href="{{ url('firm/vehicles/create') }}"><i class="fa fa-plus"></i> Add Vehicle</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="body">
                    <div class="table-responsive">
                        <table style="width: 1220px;" id="vehiclesData" class="text-center table table-bordered table-striped table-hover js-basic-example dataTable">
                            <thead>
                            <tr>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            type_field = $('#form_vehicles_search input[name=type]');
            brand_field = $('#form_vehicles_search input[name=brand]');
            year_field = $('#form_vehicles_search input[name=year]');

            vehiclesTable = $('#vehiclesData').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                "order": [[ 0, "asc" ]],
                ...
                ajax: {
                    url:"{{ url('/firm/vehicles/get-vehicles-data') }}",
                    method: 'POST',
                    dataType: "JSON",
                    headers: {
                    ...
                    data: function (d) {
                        //for search
                        d.type = type_field.val();
                        d.brand = brand_field.val();
                        d.year = year_field.val();
                    }
                },
                type: 'GET',
                columns: [
                    ...
                    {
                        data: 'no_of_passengers', name: 'no_of_passengers',
                        "searchable": true,
                        "sortable": true,
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            $(nTd).html(
                                "<a href='{{url('firm/vehicles/')}}/" + oData.id + "/edit' class='btn btn-xs btn-primary' title='Edit'><i class='fa fa-edit'></i></a>  "+
                                "<a href='javascript:' url='{{url('firm/vehicles/delete/')}}/" + oData.id + "' onclick='destroy(" + oData.id + ")' id='delete_" + oData.id + "' class='btn btn-xs btn-danger' title='Delete'><i class='fa fa-trash-alt'></i></a>"
                            );
                        }
                    }
                ]
            ...
            $('#form_vehicles_search').on('submit', function(e) {
                vehiclesTable.draw();
                e.preventDefault();
            });
            ...
            $('#clear_button').on('click', function(e) {
                ...
                brand_field.val("");
                year_field.val("");
                $('#form_vehicles_search').submit();
                check_inputs();
            });

            type_field.on("keyup", function () {
                check_inputs();
            });
            brand_field.on("keyup", function () {
                check_inputs();
            });
            year_field.on("keyup", function () {
                check_inputs();
            });

            function check_inputs(){
                if (type_field.val().length > 0 || brand_field.val().length > 0 || year_field.val().length > 0){
