...
@section('title') Mission priorities @endsection
...
@section('breadcrumbs')
    <div class="row">
        <div class="col-sm-3">
            <h4 class="ml-4 mt-3"><i class="fa fa-list-ol fa-1x"></i> Mission Priorities</h4>
        </div>
        <div class="col-sm-9">
            <ul class="breadcrumb">
                <li class="breadcrumb-item float-md-right ml-3 mr-4">
                    <p>{{ Breadcrumbs::render('mission-priorities') }}</p>
                </li>
            </ul>
        </div>
    </div>
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="body row" style="margin-bottom: -15px">
                    <div class="col-sm-9">
                        <form method="get" id="form_priorities_search">
                            <div class="row clearfix">
                    ...
                    <div class="col-sm-2 m-auto">
                        <a style="float: right; margin-top: 5px; margin-bottom: 20px" class="btn btn-success" href="{{ url('firm/mission-priorities/create') }}"><i class="fa fa-plus"></i> Add Priority</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                <div class="body">
                    <div class="table-responsive">
                        <table style="width: 1220px;" id="prioritiesData" class="text-center table table-bordered table-striped table-hover js-basic-example dataTable">
                            <thead>
                            <tr>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            _name = $('#form_priorities_search input[name=name]');
            _order = $('#form_priorities_search input[name=order]');

            prioritiesTable = $('#prioritiesData').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                "order": [[ 0, "asc" ]],
                ...
                ajax: {
                    url:"{{ url('/firm/mission-priorities/get-priorities-data') }}",
                    method: 'POST',
                    dataType: "JSON",
                    headers: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            $(nTd).html(
                                "<a href='{{url('firm/mission-priorities/')}}/" + oData.id + "/edit' class='btn btn-xs btn-primary' title='Edit'><i class='fa fa-edit'></i></a>  "+
                                "<a href='javascript:' url='{{url('firm/mission-priorities/delete/')}}/" + oData.id + "' onclick='destroy(" + oData.id + ")' id='delete_" + oData.id + "' class='btn btn-xs btn-danger' title='Delete'><i class='fa fa-trash-alt'></i></a>"
                            );
                        }
                    }
                ]
            ...
            $('#form_priorities_search').on('submit', function(e) {
                prioritiesTable.draw();
                e.preventDefault();
            });
