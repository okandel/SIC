...
@section('title') Client branches @endsection
...
@section('breadcrumbs')
    <div class="row">
        <div class="col-sm-3">
            <h4 class="ml-4 mt-3"><i class="fa fa-map-marked-alt fa-1x"></i> Client Branches</h4>
        </div>
        <div class="col-sm-9">
            <ul class="breadcrumb">
                <li class="breadcrumb-item float-md-right ml-3 mr-4">
                    <p>{{ Breadcrumbs::render('client-branches', $client) }}</p>
                </li>
            </ul>
        </div>
    </div>
                ...
                <div class="card">
                    <div class="body row" style="margin-bottom: -15px">
                        ...
                        <div class="col-sm-2 m-auto">
                            <a style="float: right; margin-top: 5px; margin-bottom: 20px" class="btn btn-success" href="{{ url('firm/client/'.$ClientId.'/branches/create') }}"><i class="fa fa-plus"></i> Add Branch</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card">
                    <div class="body">
                        <div class="table-responsive">
                            <table style="width: 1220px;" id="branchesData" class="text-center table table-bordered table-striped table-hover js-basic-example dataTable">
                                <thead>
                                <tr>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            ...
            d_name = $('#form_branches_search input[name=display_name]');
            c_person = $('#form_branches_search input[name=contact_person]');

            branchesTable = $('#branchesData').DataTable({
                processing: true,
                serverSide: true,
                searching: false,
                "order": [[ 0, "asc" ]],
                ...
                ajax: {
                    url:"{{ url('/firm/client/'.$ClientId.'/branches/get-branches-data') }}",
                    method: 'POST',
                    dataType: "JSON",
                    headers: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                            $(nTd).html(
                                "<a href='{{url('firm/client/')}}/"+ {{$ClientId}} + "/branches/" + oData.id + "/edit' class='btn btn-xs btn-primary' title='Edit'><i class='fa fa-edit'></i></a>  "+
                                "<a href='javascript:' url='{{url('firm/client/')}}/"+ {{$ClientId}} + "/branches/" + oData.id + "/delete' onclick='destroy(" + oData.id + ")' id='delete_" + oData.id + "' class='btn btn-xs btn-danger' title='Delete'><i class='fa fa-trash-alt'></i></a>"
                            );
                        }
                    }
                ]
            ...
            function check_inputs(){
                if (d_name.val().length > 0 || c_person.val().length > 0){
